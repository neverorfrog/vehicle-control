...
class RacingSimulator(Simulator):
    ...
    def __init__(
        ...
        point_masses = [
            models.DynamicPointMass(config=carconfig, track=self.track)
            for _ in self.names
        ]
        for car in cars:
        ...
        # controllers
        controllerconfigs = [
            OmegaConf.create(utils.load_config(f"config/controllers/{name}.yaml"))
        ...
        controllers = [
            control.CascadedMPC(car=car, point_mass=point_mass, config=config)
            for car, point_mass, config in combriccola
        ]
    ...
    def init_animation(self, func: object, fig: Figure = plt.gcf(), frames: int = None):
        ...
        if self.slip_angle_plot is True and len(self.names) == 1:
            ...
            self.ax_small0.set_ylabel(
                r"$\alpha_f$", fontsize=16, labelpad=25, rotation=360, color="blue"
            )
            ...
            self.ax_small1.set_ylabel(
                r"$\alpha_r$", fontsize=16, labelpad=25, rotation=360, color="blue"
            )
        ...
        # the dynamics of the state
            ...
            self.ax_small4.set_ylabel(
                r"$F_x \rightarrow N$", fontsize=16, labelpad=25, rotation=360
            )
        ...
        if self.loaded:
            return FuncAnimation(
                fig, func, frames, interval=0, cache_frame_data=False, repeat=False
            )
        else:
            return FuncAnimation(
        ...
        for j in range(len(self.names)):
            ...
            if self.slip_angle_plot is True and len(self.names) == 1:
                alpha_f = ca.fabs(
                    np.rad2deg(
                        car.alpha_f(
                            state[car.state.index("Ux")],
                            state[car.state.index("Uy")],
                            state[car.state.index("r")],
                            state[car.state.index("delta")],
                        )
                        .full()
                        .squeeze()
                        .item()
                ...
                alphamod_f = ca.fabs(
                    np.rad2deg(car.alphamod_f(Fx[n]).full().squeeze().item())
                )
                ...
                alphamod_r = ca.fabs(
                    np.rad2deg(car.alphamod_r(Fx[n]).full().squeeze().item())
                )
    ...
    def step(self, controller: Controller, car: models.DynamicCar) -> Union[None, tuple]:
        try:
