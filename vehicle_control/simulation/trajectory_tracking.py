...
class TrajectoryTrackingSimulation:
    def __init__(
        self, name: str, robot: Robot, controller: Controller, trajectory: Trajectory
    ):
        self.name = name
        self.robot = robot
    ...
    def animate(
        ...
        elapsed_time = plt.gcf().text(
            0.4, 0.9, "Average computation time", fontsize=16, ha="center", va="center"
        )
        ...
        def update(i):
            ...
            ax_small1.plot(
                time[:i], state_traj[:i, :-1], "-", alpha=0.7, label=["x", "y", "psi"]
            )
            ax_small1.legend()
            ...
            ax_small3.plot(
                time[:i], error_traj[:i, :], "-", alpha=0.7, label=["e_x", "e_y"]
            )
            ax_small3.legend()
