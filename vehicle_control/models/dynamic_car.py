...
class DynamicCar(RacingCar):
    ...
    def _init_model(self):
        ...
        Fy_f = ca.if_else(
            ...
            -Calpha_f * tan(alpha_f)
            + Calpha_f**2 * fabs(tan(alpha_f)) * tan(alpha_f) / (3 * Fymax_f)
            - (Calpha_f**3 * tan(alpha_f) ** 3) / (27 * Fymax_f**2),  # first case,
            -Calpha_f * (1 - 2 * eps + eps**2) * tan(alpha_f)
            - Fymax_f * (3 * eps**2 - 2 * eps**3) * sign(alpha_f),
        )
        ...
        Fy_r = ca.if_else(
            ...
            -Calpha_r * tan(alpha_r)
            + Calpha_r**2 * fabs(tan(alpha_r)) * tan(alpha_r) / (3 * Fymax_r)
            - (Calpha_r**3 * tan(alpha_r) ** 3) / (27 * Fymax_r**2),
            -Calpha_r * (1 - 2 * eps + eps**2) * tan(alpha_r)
            - Fymax_r * (3 * eps**2 - 2 * eps**3) * sign(alpha_r),
        )
        ...
        state_dot = ca.vertcat(
            Ux_dot, Uy_dot, r_dot, delta_dot, s_dot, ey_dot, epsi_dot, t_dot
        )
