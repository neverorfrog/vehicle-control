...
class RacingCar(Robot):
    ...
    def drive(self, input: FancyVector):
        ...
        next_state = (
            self.transition(self.state.values, input.values, curvature, self.dt)
            .full()
            .squeeze()
        )
    ...
    def plot(self, axis: Axes, state: FancyVector, color="blue"):
        ...
        rectangle = plt.Rectangle(
            xy=(
                x - np.cos(angle) * width / 2 - np.cos(psi) * 2 * width / 3,
                y - np.sin(angle) * height / 2 - np.sin(psi) * 2 * height / 3,
            ),
            width=width,
            height=height,
            alpha=0.7,
        ...
        wheel_right_front = plt.Rectangle(
            (x + np.cos(angle) * r, y + np.sin(angle) * r),
            width=wheel_width,
            height=wheel_height,
        ...
        wheel_left_front = plt.Rectangle(
            (
                x - np.cos(angle) * r - cos(wheel_angle) * wheel_width,
                y - np.sin(angle) * 0.9 * r - sin(wheel_angle) * wheel_width,
            ),
            width=wheel_width,
            height=wheel_height,
            angle=np.rad2deg(wheel_angle),
        ...
        wheel_right_back = plt.Rectangle(
            (
                x + np.cos(angle) * r - np.cos(psi) * width * 0.6,
                y + np.sin(angle) * r - np.sin(psi) * height * 0.6,
            ),
            width=wheel_width,
            height=wheel_height,
            angle=np.rad2deg(wheel_angle),
        ...
        wheel_left_back = plt.Rectangle(
            (
                x
                - np.cos(angle) * r
                - np.cos(psi) * width * 0.6
                - cos(wheel_angle) * wheel_width,
                y
                - np.sin(angle) * r * 0.9
                - np.sin(psi) * height * 0.6
                - sin(wheel_angle) * wheel_width,
            ),
            width=wheel_width,
            height=wheel_height,
            angle=np.rad2deg(wheel_angle),
