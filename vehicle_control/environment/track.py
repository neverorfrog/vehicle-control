...
class Obstacle:
    ...
    def plot(self, axis: Axes):
        ...
        circle = plt_patches.Circle(
            xy=(self.cx, self.cy), radius=self.radius, color="#2E4053", zorder=20
        )
...
class Track:
    def __init__(self, config: OmegaConf):
        ...
        self.x, self.y, self.dx_ds, self.dy_ds, self.ddx_ds, self.ddy_ds = (
    ...
    def _construct_obstacles(self, obstacle_data):
        ...
        self.occupancy: ca.Function = ca.interpolant(
            "occupancy", "linear", [s_values, ey_values], data_flat, {"inline": True}
        )
        return obstacles
    ...
    def _precompute_curvatures(self):
        ...
        k_spline = ca.interpolant(
            "curvatures", "bspline", [s_values], curvatures, {"degree": [3]}
        )
        s = ca.MX.sym("s")
    ...
    def _divide_track(self):
        ...
        for s in s_values:
            ...
            # curve ends
            if abs(curv) < eps and is_curve is True:
                if s - start > 1:
                    is_curve = False
    ...
    def _construct_spline(self):
        ...
        compute_segment_length = (
            lambda s: np.sqrt(dx_ds(s) ** 2 + dy_ds(s) ** 2).full().squeeze()
        )
        self.length = trapezoid(
            compute_segment_length(one_lap_range), one_lap_range, dx=0.1
        )
    ...
    def _construct_path(self, corners):
        ...
        for wp_id in range(0, len(wp_x)):
            if wp_id < self.smoothing:
                ...
                wp_xs.append(
                    np.mean(wp_x[wp_id - self.smoothing : wp_id + self.smoothing + 1])
                )
                wp_ys.append(
                    np.mean(wp_y[wp_id - self.smoothing : wp_id + self.smoothing + 1])
                )
