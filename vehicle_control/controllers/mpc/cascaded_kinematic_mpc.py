...
from vehicle_control.models.kinematic_car import KinematicCar
...
class CascadedKinematicMPC(Controller):
    ...
    def _init_variables(self):
        ...
        self.state0 = (
            ...
            if self.N > 0
            else self.opti.parameter(self.ns_kc)
        )
    ...
    def _stage_cost(self, n):
        ...
        cost += ca.if_else(
            fabs(tan(self.car.alpha_f(Ux, Uy, r, delta)))
            >= tan(self.car.alphamod_f(Fx)),  # slip angle front
            cost_weights.slip
            * (
                fabs(tan(self.car.alpha_f(Ux, Uy, r, delta)))
                - tan(self.car.alphamod_f(Fx))
            )
            ** 2,
            0,
        )
        ...
        cost += ca.if_else(
            fabs(tan(self.car.alpha_r(Ux, Uy, r, delta)))
            >= tan(self.car.alphamod_r(Fx)),  # slip angle rear
            cost_weights.slip
            * (
                fabs(tan(self.car.alpha_r(Ux, Uy, r, delta)))
                - tan(self.car.alphamod_r(Fx))
            )
            ** 2,
            0,
        )
        ...
        if self.config.obstacles:  # Obstacle avoidance
            for obs in self.car.track.obstacles:
                distance = ca.fabs(
                    ca.sqrt((s - obs.s) ** 2 + (ey - obs.ey) ** 2) - (obs.radius)
                )
    ...
    def _kc_stage_constraints(self, k):
        ...
        self.opti.subject_to(
            self.opti.bounded(
                state_kc_constraints.delta_min, delta, state_kc_constraints.delta_max
            )
        )

        # input limits
        self.opti.subject_to(
            self.opti.bounded(input_constraints.a_min, a, input_constraints.a_max)
        )
        self.opti.subject_to(
            self.opti.bounded(input_constraints.w_min, w, input_constraints.w_max)
        )
        ...
        if k < self.H - 1:
            self.opti.subject_to(
                self.state[: self.ns_kc, k + 1]
                == self.kin_car.spatial_transition(
                    state, action, self.curvature[k], self.ds[k]
                )
            )
    ...
    def _kc_stage_cost(self, k):
        ...
        cost += ca.if_else(
            ey < state_kc_constraints.ey_min,  # 1) road boundary intrusion
            cost_weights.boundary * ds * (ey - state_kc_constraints.ey_min) ** 2,
            0,
        )

        cost += ca.if_else(
            ey > state_kc_constraints.ey_max,  # 2) road boundary intrusion
            cost_weights.boundary * ds * (ey - state_kc_constraints.ey_max) ** 2,
            0,
        )

        cost += (
            cost_weights.deviation_kc * ds * (ey**2)
    ...
    def _switching_cost(self):
        cost_weights = self.config.cost_weights
        Ux, Uy, r, delta, s, ey, epsi, t = self._unpack_state(
        ...
        return (cost_weights.Fy / self.ds[self.N]) * (
            (a_final - a_initial) ** 2 + (w_final - w_initial) ** 2
        )
    ...
    def _switching_constraints(self):
        ...
        self.opti.subject_to(
            state_kc_initial[self.kin_car.state.index("delta")] == delta_final
        )
    ...
    def _terminal_cost(self):
        ...
        cost += (
            cost_weights.ey * final_state[final_model.state.index("ey"), -1] ** 2
        )  # final cost (minimize terminal lateral error) hardcodato
        cost += (
            cost_weights.epsi * final_state[final_model.state.index("epsi"), -1] ** 2
    ...
    def _init_horizon(self, state):
        ...
        ds_kc_traj = (
            np.full(self.K, self.config.mpc_dt_kc)
            * self.state_prediction[self.kin_car.state.index("v"), self.N : self.H]
        )
    ...
    def get_state_prediction(self):
        ...
        preds_kin = (
            [
                self.kin_car.rel2glob(self.state_prediction[:, k])
                for k in range(self.N, self.H)
            ]
            if self.K > 0
            else []
