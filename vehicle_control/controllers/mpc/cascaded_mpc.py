...
import casadi as ca
import numpy as np
from casadi import cos, fabs, tan
...
class CascadedMPC(Controller):
    ...
    def _init_variables(self):
        ...
        self.action_prediction = np.ones((self.na, self.H)) + np.random.random(
            (self.na, self.H)
        )
        ...
        # initial state
        self.state0 = (
            self.opti.parameter(self.ns)
            if self.N > 0
            else self.opti.parameter(self.ns_pm)
        )

    def _stage_constraints(self, n):
        state = self.state[: self.ns, n]
    ...
    def _stage_cost(self, n):
        ...
        cost += ca.if_else(
            talpha_f >= talphamod_f, cost_weights.slip * (talpha_f - talphamod_f) ** 2, 0
        )  # slip angle front

        talpha_r = fabs(tan(self.car.alpha_r(Ux, Uy, r, delta)))
        talphamod_r = tan(self.car.alphamod_r(Fx))
        cost += ca.if_else(
            talpha_r >= talphamod_r, cost_weights.slip * (talpha_r - talphamod_r) ** 2, 0
        )  # slip angle rear

        if n < self.N - 1:  # Force Action Continuity
            next_action = self.action[:, n + 1]
            cost += (cost_weights.Fx / ds) * (
                next_action[self.car.input.index("Fx")] - Fx
            ) ** 2
    ...
    def _pm_stage_constraints(self, m):
        ...
        if m < self.H - 1:
            self.opti.subject_to(
                self.state[: self.ns_pm, m + 1]
                == self.point_mass.spatial_transition(
                    state, action, self.curvature[m], self.ds[m]
                )
            )
    ...
    def _pm_stage_cost(self, m):
        ...
        cost += ca.if_else(
            ey < state_pm_constraints.ey_min,  # 1) road boundary intrusion
            cost_weights.boundary * ds * (ey - state_pm_constraints.ey_min) ** 2,
            0,
        )

        cost += ca.if_else(
            ey > state_pm_constraints.ey_max,  # 2) road boundary intrusion
            cost_weights.boundary * ds * (ey - state_pm_constraints.ey_max) ** 2,
            0,
        )

        cost += (
            cost_weights.deviation_pm * ds * (ey**2)
        )  # 3) deviation from road descriptor path
    ...
    def _switching_cost(self):
        cost_weights = self.config.cost_weights
        Ux_final, Uy_final, r_final, delta_final, _, _, _, _ = self._unpack_state(
            self.state[: self.ns, self.N - 1]
        )  # final state
        ...
        return (
            cost_weights.switch_F
            * (1 / self.ds[self.N - 1])
            * (((Fx_bar_initial - Fx_final) ** 2) + (Fy_bar_initial - Fy_f - Fy_r) ** 2)
        )

    def _switching_constraints(self):
        Ux_final, Uy_final, _, _, s_final, ey_final, epsi_final, t_final = (
            self._unpack_state(self.state[:, self.N - 1])
        )  # final state
        state_pm_initial = self.state[:, self.N]  # initial state of point mass
        self.opti.subject_to(
            state_pm_initial[self.point_mass.state.index("V")]
            == (Ux_final**2 + Uy_final**2) ** 0.5
        )
        self.opti.subject_to(
            state_pm_initial[self.point_mass.state.index("ey")] == ey_final
        )
        self.opti.subject_to(
            state_pm_initial[self.point_mass.state.index("epsi")]
            == ca.atan(Uy_final / Ux_final) + epsi_final
        )
        self.opti.subject_to(
            state_pm_initial[self.point_mass.state.index("s")] == s_final
        )
        self.opti.subject_to(
            state_pm_initial[self.point_mass.state.index("t")] == t_final
        )
    ...
    def _terminal_cost(self):
        ...
        cost += ca.if_else(
            final_speed >= state_constraints.max_speed,
            cost_weights.speed * (final_speed - state_constraints.max_speed) ** 2,
            0,
        )  # excessive speed
        cost += cost_weights.time * (
            final_state[final_model.state.index("t")]
        )  # final cost (minimize time)
        cost += (
            cost_weights.ey * final_state[final_model.state.index("ey")] ** 2
        )  # final cost (minimize terminal lateral error) hardcodato
        cost += (
            cost_weights.epsi * final_state[final_model.state.index("epsi")] ** 2
    ...
    def command(self, state) -> Tuple[FancyVector, FancyVector]:
        ...
        action = DynamicCarAction(
            Fx=self.action_prediction[0][0], w=self.action_prediction[1][0]
        )
        return action
    ...
    def _init_horizon(self, state):
        ...
        ds_traj = (
            np.full(self.N, self.config.mpc_dt)
            * self.state_prediction[self.car.state.index("Ux"), : self.N]
        )
        ...
        ds_pm_traj = np.full(
            self.M, self.config.ds_pm
        )  # self.config.mpc_dt_pm) * self.state_prediction[self.point_mass.state.index('V'),self.N:self.H]
    ...
    def get_state_prediction(self):
        preds_car = [
            self.car.rel2glob(self.state_prediction[:, i]) for i in range(self.N)
        ]
        preds_pm = (
            [
                self.point_mass.rel2glob(self.state_prediction[:, j])
                for j in range(self.N, self.H)
            ]
            if self.M > 0
            else []
        )
