...
\paragraph{$F_x$ usage}
The average $F_x$ cannot be analysed separately from the other parameters of the 
table \ref{simulations}. So, let's take firstly in consideration just the first 
two simulations, and then we will extend the considerations to the other two. 
The single-track model is the one that takes the longest time to finish the circuit,
even if it has both higher average speed and higher average force than the other model.
This means that the cascaded model can optimize in a better way its velocity and the 
distribution of the force along the track, due to its long horizon planning capacity.\\
Consider now the third simulation data. It descreases the lap time with respect to
the first two, having also an higher speed. The applied force is a little bit lower than
the one applied on the single-track. This overview suggests how this cascaded model is the
best one between the three considered, because it can balance the usage of the force in 
order to reach better performances.\\
The same argument can be applied to the last cascaded model, which has the longest horizon
window.

\paragraph{Slip angle excess}
\todo[inline]{slip angle}

\paragraph{Laptime weight vs boundary weight}
\todo[inline]{laptime weight vs boundary weight}

\paragraph{Weight on switch from singletrak to pointmass}
\todo[inline]{switch from singletrak to pointmass}
