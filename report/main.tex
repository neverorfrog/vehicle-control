...
computationally cost-effective approach for a real-time NMPC for an autonomous
...
of the vehicle. 
\paragraph*{} The same idea is proposed also in Model Hierarchy Predictive controllers (MHPC) in \cite{MHPC},
where a single optimization problem is built over a hierarchy of models. The transition
between the models is enforced by a projection map constraint of the higher-order model into
the lower-order model, that ensures consistency between them. This concept has found an 
application also in humanoid robotic systems, such as in \cite{humanoid}, where MHPC was used
for arm motion planning capable to react to disturbances and anticipate future states. 
In particular, the first model is a whole-body kino-dynamic model, while the second one is 
a single rigid body model, that is used to extend the prediction horizon with a low computational time. 
