...
where $\tilde{\chi}_f$ and $\tilde{\chi}_r$ are scalars that sum to 1, however, these equations will be used for numerical optimization, and should therefore be twice differentiable, and we therefore have to use the workaround equations presented above.
...
The Point-Mass model is a simpler version of the Single-Track model, where there is only a velocity state $V$, and three position states relative to the road descriptor path: the distance $s$ along it, the lateral distance $e$ from it, and finally the difference in orientation  $\phi$ between the velocity vector and the tangent to the road descriptor path.

With the ... of substituting ${U}_x$ with $V$ we use the same equations introduced in the single-track model, in particular for the drag force ${F}_d$ we use equation \ref{}, for ${F}_b$ \ref{} and for the normal loads ${F}_{z_f}$, ${F}_{z_r}$ \ref{}. Following the basic intuition of having to simplify the model, the tire forces are not modeled.

The inputs to this model are the longitudinal force ${F}_x$ and the lateral force ${F}_y$, finally we can define the following equations of motion:

\begin{equation}
    \dot{V} = \frac{F_{x}-F_{d}}{m}
\end{equation}
\begin{equation}
    \dot{s} = \frac{V\cos(\phi)}{1-\kappa e}
\end{equation}
\begin{equation}
    \dot{e} = V\sin(\phi)
\end{equation}
\begin{equation}
    \dot{\phi} = \frac{{F}_y + {F}_b}{mV} - k\dot{s}
\end{equation}
...
Here we define a mapping from the single-track to the point-mass states. We add a line on top of the point-mass states for clarity.
...
\end{equation}
\begin{equation}
    \bar{\phi} = arctan(\frac{{U}_y}{{U}_x}) + \Delta\psi
\end{equation}
