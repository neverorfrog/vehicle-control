...
\paragraph*{} Due to the duality of the requirements, a popular approach has
been to use different control loops to address different tasks. In
\cite{Gros2007} for example, to control a quadcopter with similar requirements
to our case, two loops with different time scales were used: a slow trajectory
generation loop and a faster loop for controlling said quadcopter through linear
feedback. \paragraph*{} An orthogonal approach, adopted in \cite{rosolia}, is to
use a linearized model that learns iteratively from a precomputed time-optimal
trajectory. \paragraph*{} Another approach has been to use two models of
different complexity, to deal with different tasks. In \cite{Gao2010} a
point-mass model is used for high-level path planning, intrinsically more
far-sighted, while a more complex four-wheel model is used for low-level path
following. The main limitation of this approach is that the trajectory generated
by the high-level path planner might be feasible for the simple point-mass model
but impossible to track by the low-level controller, that has to control a more
complex model, with more constraints and limitations. \paragraph*{}\cite{paper}
follows the intuition of using models of different complexity, and improves on
it by employing a so-called \textbf{cascaded model}, composed by a detailed
model of the car for the near term, and a simpler model used for planning in the
long term. This leads to the developement a novel, more computationally
cost-effective approach for a real-time NMPC for an autonomous driving, which
...
\paragraph*{} The same idea is proposed also in Model Hierarchy Predictive
controllers (MHPC) in \cite{MHPC}, where a single optimization problem is built
over a hierarchy of models. Considering a quadruped, the simpler model neglects
the presence of legs, reducing the full body model to a trunk model. This is
very similar to the approach used in this work, where the simpler model neglects
the presents of wheels. In the quadruped, the transition between the models is
enforced by a projection map constraint of the higher-order model into the
lower-order model, that ensures consistency between them. This projection map is
needed to guide the full body model towards actions that are favorable for the
long-term, without the burden of using the full body model itself for the whole
horizon. This concept has found an application also in humanoid robotic
systems, such as in \cite{humanoid}, where MHPC was used for arm motion planning
capable to react to disturbances and anticipate future states. In particular,
the first model is a whole-body kino-dynamic model, while the second one is a
single rigid body model, that is used to extend the prediction horizon with a
...
The inputs to this model are the longitudinal force $\bar{F}_x$ and the lateral force $\bar{F}_y$.
...
\begin{figure}[H] 
    \centering
        ...
        \subcaption{Singletrack with $N=50$, Cascaded with $N=20, M=15$}
    \end{subfigure}
    \begin{subfigure}{0.85\textwidth}
        \centering
        \includegraphics[width=\textwidth]{assets/im/fig2_60.png}
        \subcaption{Singletrack with $N=60$, Cascaded with $N=20, M=35$}
    \end{subfigure}
...
\begin{figure}[H]
    ...
    \begin{subfigure}{0.8\textwidth}
        ...
        \subcaption{Cascaded with $N=20, M=15, V_{max}=18$,\\ Singletrack with $N=50, V_{max}=18$}
        \label{race1}
    \end{subfigure}
    \begin{subfigure}{0.8\textwidth}
        \centering
        \includegraphics[width=\textwidth]{assets/im/race7.png}
        \subcaption{Cascaded with $N=20, M=35, V_{max}=18$, \\ Singletrack with $N=50, V_{max}=20$}
        \label{race7}
    \end{subfigure}
    \begin{subfigure}{0.8\textwidth}
        \centering
        \includegraphics[width=\textwidth]{assets/im/race5.png}
        \subcaption{Cascaded with $N=20, M=35, V_{max}=18$, \\ Singletrack with $N=60, V_{max}=20$}
        \label{race5}
    \end{subfigure}
...
\begin{figure}
    ...
    \begin{subfigure}{0.76\textwidth}
        \centering
        \includegraphics[width=\textwidth]{assets/im/cascaded_slip_35_18.png}
        \subcaption{Cascaded with $N=20, M=35, V_{max}=18$}
        \label{cascaded_slip}
    \end{subfigure}
    \begin{subfigure}{0.76\textwidth}
        \centering
        \includegraphics[width=\textwidth]{assets/im/singletrack_slip_50_20.png}
        \subcaption{Singletrack with $N=50, V_{max}=20$}
        \label{single_slip_50_20}
    \end{subfigure}
    \begin{subfigure}{0.76\textwidth}
        \centering
        \includegraphics[width=\textwidth]{assets/im/singletrack_slip_60_18.png}
        \subcaption{Singletrack with $N=60, V_{max}=18$}
        \label{single_slip_60_18}
    \end{subfigure}
    \begin{subfigure}{0.76\textwidth}
        \centering
        \includegraphics[width=\textwidth]{assets/im/singletrack_slip_60_20.png}
        \subcaption{Singletrack with $N=60, V_{max}=20$}
        \label{single_slip_60_20}
    \end{subfigure}
    \caption[short]{Comparisons of slip angles}
    \label{slip_angles}
\end{figure}
