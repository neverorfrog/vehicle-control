...
The car can be model as a single-track, where the two tires are merged together in just one.
For the intrinsic properties, the vehicle is assumed to have a mass $m$ and a yaw moment of
inertia $I_{zz}$. The state of the model is composed by the longitudinal speed $U_x$, the 
lateral speed $U_x$ and the yaw rate $r$ of the Center of Gravity, then by the curvilinear
coordinate $s$, the lateral distance $e$ and the orientation of the chassis $\Delta \psi$
with respect to che road descriptor path 
